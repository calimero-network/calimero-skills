# Manifest Format (V2)

The manifest describes the app bundle. It is generated by `calimero-registry bundle create`
and can optionally be signed with `mero-sign` before pushing.

## Bundle create generates this automatically

When you run `calimero-registry bundle create`, the CLI generates a `manifest.json` inside
the bundle directory. You can also supply a manifest via `-m, --manifest <path>` to pre-fill fields.

## Manifest V2 format

```json
{
  "version": "1.0",
  "package": "com.example.myapp",
  "appVersion": "1.0.0",
  "metadata": {
    "name": "My Application",
    "description": "Application description",
    "author": "Your Name"
  },
  "wasm": {
    "path": "app.wasm",
    "hash": "sha256:...",
    "size": 12345
  },
  "links": {
    "frontend": "https://example.com",
    "github": "https://github.com/example/myapp",
    "docs": "https://example.com/docs"
  },
  "minRuntimeVersion": "0.3.0"
}
```

## After signing with mero-sign

mero-sign injects a `signature` block:

```json
{
  "version": "1.0",
  "package": "com.example.myapp",
  ...
  "signature": {
    "alg": "ed25519",
    "pubkey": "yuKE404BaldXazEIUC4XrVGFyXxxyoRVjrrGhcKk1P4",
    "sig": "base64url-64-bytes",
    "signedAt": "2026-03-13T12:00:00Z"
  }
}
```

## Package ownership

- The first push establishes the package owner via the Ed25519 `signature.pubkey`
- Only the owner (or keys in `manifest.owners`) can push subsequent versions
- For team publishing, add teammates' public keys to `manifest.owners`:

```json
{
  "version": "1.0",
  "package": "com.example.myapp",
  "owners": [
    "yuKE404BaldXazEIUC4XrVGFyXxxyoRVjrrGhcKk1P4",
    "anotherTeammatePubKey..."
  ],
  ...
}
```

## Package naming

| Rule | Example |
| --- | --- |
| Must be reverse-domain | `com.yourorg.appname` ✅ |
| Version must be full SemVer | `1.0.0` ✅ |
| No `v` prefix on version | `v1.0.0` ❌ |

## `links.frontend` is used by Desktop

The Desktop app reads `links.frontend` to know which URL to open when a user opens this app.
Always include it.
