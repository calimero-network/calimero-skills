...
| `WriterSetCell<T>`        | Shared value gated by a writer set    | Authenticated, rotatable writer set; prefer over map+max-wins (0.11) |
...
> slot author ownership; `WriterSetCell` (the `shared` module's type — there is no `SharedStorage`
> collection) is a shared value gated by an authenticated, rotatable writer set.
