...
- A `CalimeroBytes` class for handling `Vec<u8>` / byte array types (emitted only when a type uses bytes)
...
> Enums with payloads instead emit a `{Name}Payload` discriminated union plus a factory
> object — e.g. `export type ActionPayload = | { name: 'Move'; payload: ... }` with
